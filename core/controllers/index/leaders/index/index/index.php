...
function index()
{
     ...
     */
    $fio_filter = ($_POST && isset($_POST['fio_filter']) ? main_checkChars($_POST['fio_filter']) : '');
    $city_filter = ($_POST && isset($_POST['city_filter']) ? main_checkChars($_POST['city_filter']) : '');
    $a_z = ($_POST && isset($_POST['a_z']) ? main_checkChars($_POST['a_z']) : '');
    $z_a = ($_POST && isset($_POST['z_a']) ? main_checkChars($_POST['z_a']) : '');
    ...
    if (!isset($_SESSION['role'])) {
        ...
        $want_filter = ($_POST && isset($_POST['want_filter']) ? main_checkChars($_POST['want_filter']) : '');
        $can_filter = ($_POST && isset($_POST['can_filter']) ? main_checkChars($_POST['can_filter']) : '');
        $help_to_me = ($_POST && isset($_POST['help_to_me']) ? main_checkChars($_POST['help_to_me']) : '');
        $i_can_help = ($_POST && isset($_POST['i_can_help']) ? main_checkChars($_POST['i_can_help']) : '');
    }

    /**
     * get all leader data for the filter
     */
    $data['filter'] = leaders_getFilters();
    ...
    if (!empty($_POST) && ($fio_filter != 'all' || $city_filter != 'all' || $want_filter != 'all' || $can_filter != 'all')) {
        $where = leaders_getWhereForFilter($_POST);
     ...
     */
    $data['css'][] = 'index/css/leaders/index/style.css';
    $data['css'][] = 'index/css/leaders/index/media.css';
    $data['js'][] = 'index/js/leaders/index/script.js';
    ...
    if (empty($_POST) && empty($_SESSION['save_data_leaders_redirect'])) {
         ...
         */
        $data['leader'] = leader_get('', ' ORDER BY l.id_lid ASC ', $limit);

        /**
         * Require view
         */
        renderView('index/leaders/index/index/index/index', $data);
    } elseif (!empty($_POST) && !empty($_SESSION['save_data_leaders_redirect']) && !isset($_POST['redir_url'])) {

        /**
         * Get data on leaders according to the conditions and the established limit
         */
        $data['leader'] = leader_get(" WHERE ".$where_str, $order_by, $limit, $want_need, $i_want, $i_can);

        /**
         * Require view
         */
        renderView('index/leaders/index/index/layouts/index', $data);
    } elseif (!empty($_POST) && !empty($_SESSION['save_data_leaders_redirect']) && isset($_POST['redir_url'])) {
