...
function index() {
    if(isset($_SESSION['role']) && $_SESSION['role'] == 'admin') {
        ...
        if (empty($_POST['familya']) || empty($_POST['name']) || empty($_POST['city']) ||empty($_POST['birthday'])) exit("empty");

        $isChangedUser = false;

        $userBefore = getData(dbQuery("SELECT image_name, familya, name, otchestvo, city, birthday, social, video, telephone, email, contact_info FROM leaders WHERE id_lid = '" . $_POST['id_lid'] . "'"));
        $fileBefore = getData(dbQuery("SELECT id, title, filename, size, ext, description FROM leaders_uploads WHERE deleted IS NULL AND id_lid = '" . $_POST['id_lid'] . "'"));
        $linksBefore = getData(dbQuery("SELECT id, title, link FROM leaders_links WHERE deleted IS NULL AND id_lid = '" . $_POST['id_lid'] . "'"));
...
//        array_pop($linksBefore);
        ...
        if ($isChangedUser) {
            ...
            dbQuery("UPDATE leaders SET fio = '" . $fio . "', familya = '" . $_POST['familya'] . "', 
                    ...
                    image_name = '" . $img . "', status_info = '1', checked = '1' WHERE id_lid = '" . $_POST['id_lid'] . "'");

            // обновилась информация о пользователе  EVENT 15
                userLogs($_SESSION['id_lid'], '15');
        }

        function add_files_to_db($title, $filename, $description, $size, $ext){
            if ($filename == '') return false;
            $id = dbQuery("INSERT INTO leaders_uploads (id_lid, filename, description, title, size, ext) VALUES ('".$_POST['id_lid']."', '".$filename."', '".$description."', '".$title."', '".$size."', '".$ext."')", true);
        ...
        function add_links_to_db($title, $link){
            ...
            if($pos !== false){
                $id = dbQuery("INSERT INTO leaders_links (id_lid, link, title) VALUES ('".$_POST['id_lid']."', '".$link."', '".$title."')", true);
                userLogs($_POST['id_lid'], '11', '', '' , '', '', $id. '');
            }
        }
        ...
        setStatusAndAccessUserOnline($_POST['id_lid']);
