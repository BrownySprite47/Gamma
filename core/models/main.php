...
function getFiltersLeaders() {
    ...
    $filters['fio'] = clean(getData(dbQuery('SELECT fio FROM leaders WHERE checked != "2" AND (status="3" OR status="2") '.$visual.' ORDER BY fio')));    $filters['city'] = clean(getData(dbQuery('SELECT DISTINCT city FROM leaders WHERE status="3" OR status="2" '.$visual.' ORDER BY city')));
    return $filters;
}
        ...
        if (isset($tags_id[0]['id'])) {
            ...
            foreach ($tags['none'] as $key => $value) {
                foreach ($tags['tag_i_can'] as $key0 => $value0) {
                    if ($value['id'] == $value0['id']) unset($tags['none'][$key]);
                }
                foreach ($tags['tag_i_want'] as $key1 => $value1) {
                    if ($value['id'] == $value1['id']) unset($tags['none'][$key]);
                }
            }
        }else{
            $tags['tag_i_can'] = [];
